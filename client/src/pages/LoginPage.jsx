...
// NEW: Anime Style Moving Student Characters
function MovingWidgets() {
  ...
  useEffect(() => {
    ...
    // Make the anime heads bounce slightly to simulate walking
    ...
    <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
      {widgets.map((w, i) => (
        <div key={i} className={`widget-${i}`} style={{
          ...
          <img className="anime-avatar" src={w.img} alt="anime student" style={{ 
...
export default function LoginPage() {
  ...
  // 3D Parallax & Entrance Animations
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let payload = isLogin 
        ? { email, password } 
        : (selectedRole === 'student' ? { name, email, password, level, dailyHours: Number(dailyHours), target } : { name, email, password });
  ...
  const goBack = () => {
    if (selectedRole) {
      gsap.to(formRef.current, { opacity: 0, x: -50, duration: 0.3, ease: 'power2.in', onComplete: () => {
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '40px 24px', position: 'relative', zIndex: 1 }}>
        <div ref={containerRef} style={{ width: '100%', maxWidth: 460 }}>
          ...
          }} onMouseEnter={e => gsap.to(e.target, { x: -5 })} onMouseLeave={e => gsap.to(e.target, { x: 0 })}>
            ...
            {!selectedRole ? (
              <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  }} onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.03, boxShadow: '0 10px 30px rgba(26,63,163,0.15)' })} onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1, boxShadow: 'none' })}>
                  ...
                  }} onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.03, boxShadow: '0 10px 30px rgba(0,201,177,0.15)' })} onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1, boxShadow: 'none' })}>
              ...
              <form ref={formRef} onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {!isLogin && (
                  <div>
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input type="text" required value={name} onChange={e => setName(e.target.value)} placeholder="Full Name" style={{ width: '100%', padding: '12px 14px 12px 40px', borderRadius: 12, border: '1.5px solid #e2e8f8', background: '#f8f9ff', fontSize: 14, outline: 'none', transition: '0.3s' }} onFocus={(e) => e.target.style.borderColor = '#1a3fa3'} onBlur={(e) => e.target.style.borderColor = '#e2e8f8'} />
                    </div>
                  </div>
                )}
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" style={{ width: '100%', padding: '12px 14px 12px 40px', borderRadius: 12, border: '1.5px solid #e2e8f8', background: '#f8f9ff', fontSize: 14, outline: 'none', transition: '0.3s' }} onFocus={(e) => e.target.style.borderColor = '#1a3fa3'} onBlur={(e) => e.target.style.borderColor = '#e2e8f8'} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    ...
                    <input type={showPassword ? 'text' : 'password'} required minLength="6" value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" style={{ width: '100%', padding: '12px 44px 12px 40px', borderRadius: 12, border: '1.5px solid #e2e8f8', background: '#f8f9ff', fontSize: 14, outline: 'none', transition: '0.3s' }} onFocus={(e) => e.target.style.borderColor = '#1a3fa3'} onBlur={(e) => e.target.style.borderColor = '#e2e8f8'} />
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ position: 'relative' }}>
                          ...
                          <select value={level} onChange={e => setLevel(e.target.value)} style={{ width: '100%', padding: '12px 14px 12px 40px', borderRadius: 12, border: '1.5px solid #e2e8f8', background: '#f8f9ff', fontSize: 14, outline: 'none' }}>
                            <option value="topper">Topper</option><option value="average">Average</option><option value="last-minute">Last-Minute</option>
                          </select>
                        </div>
                      </div>
                      <div style={{ width: '40%' }}>
                ...
                }} onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.02 })} onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1 })}>
