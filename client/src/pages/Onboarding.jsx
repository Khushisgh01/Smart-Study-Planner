...
import {
  ...
  School, Loader, AlertCircle, Users,
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
...
// Total steps: 2 quiz steps + hours + class selection = 4
const TOTAL_STEPS = 4;

// ── Class colours per grade ───────────────────────────────────────────────
const CLASS_COLORS = {
  '9':  { color: '#1a3fa3', gradient: 'linear-gradient(135deg,#1a3fa3,#3a5fd4)', shadow: 'rgba(26,63,163,0.35)' },
  '10': { color: '#00c9b1', gradient: 'linear-gradient(135deg,#00c9b1,#00e6cc)', shadow: 'rgba(0,201,177,0.35)'  },
  '11': { color: '#ff2d78', gradient: 'linear-gradient(135deg,#ff2d78,#ff6ba0)', shadow: 'rgba(255,45,120,0.35)' },
  '12': { color: '#e8a020', gradient: 'linear-gradient(135deg,#e8a020,#f5c842)', shadow: 'rgba(232,160,32,0.35)' },
};
const defaultClassStyle = { color: '#9b59b6', gradient: 'linear-gradient(135deg,#9b59b6,#b97dd4)', shadow: 'rgba(155,89,182,0.35)' };
...
function FloatingBubbles() {
  const bubbles = Array.from({ length: 28 }, (_, i) => ({
    ...
    top:  `${Math.random() * 100}%`,
    color: ['rgba(26,63,163,0.06)','rgba(232,160,32,0.06)','rgba(0,201,177,0.06)','rgba(255,45,120,0.04)'][i % 4],
  }));

  useEffect(() => {
    gsap.to('.obs-bubble', {
      y: () => `random(-160,160)`, x: () => `random(-160,160)`,
      scale: () => `random(0.75,1.3)`, duration: () => `random(5,10)`,
      repeat: -1, yoyo: true, ease: 'sine.inOut', stagger: 0.08,
    });
  }, []);
...
async function savePreferences(level, target, dailyHours) {
  try {
    const token = localStorage.getItem('token');
    if (!token) return;
    await fetch('/api/auth/user/preferences', {
    ...
    console.warn('Preferences save (non-critical):', err.message);
  }
}
...
function ClassSelectionStep({ onSelect, selectedClass }) {
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        setError(`Could not load classes: ${err.message}. Make sure the server is running.`);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (!loading && classes.length > 0) {
      gsap.fromTo('.class-pick-card',
        { opacity: 0, y: 24, scale: 0.93 },
        { opacity: 1, y: 0, scale: 1, stagger: 0.08, duration: 0.45, ease: 'back.out(1.5)', delay: 0.1 }
      );
    }
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, padding: '48px 0', color: 'var(--text-muted)' }}>
        <Loader size={20} style={{ animation: 'rotateSpin 0.8s linear infinite' }} />
        <span style={{ fontSize: 14 }}>Loading available classes…</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div style={{ padding: '20px', borderRadius: 16, background: 'rgba(255,45,120,0.07)', border: '1px solid rgba(255,45,120,0.2)', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        <AlertCircle size={18} color="#ff2d78" style={{ flexShrink: 0, marginTop: 1 }} />
        <div>
          <p style={{ fontSize: 13, fontWeight: 600, color: '#ff2d78', marginBottom: 4 }}>Could not load classes</p>
  ...
  if (classes.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: '40px 24px', borderRadius: 20, border: '2px dashed var(--border)', background: 'var(--bg-primary)' }}>
        <div style={{ fontSize: 48, marginBottom: 12 }}>🏫</div>
        <p style={{ fontFamily: 'Syne,sans-serif', fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', marginBottom: 8 }}>
          No classes created yet
        </p>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.7, maxWidth: 320, margin: '0 auto' }}>
          Ask your teacher to create a class section in their dashboard, then refresh this page.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {classes.map(cls => {
        const style    = CLASS_COLORS[cls.name] || defaultClassStyle;
        const isActive = selectedClass?._id === cls._id;
        return (
          <button
            key={cls._id}
            className="class-pick-card"
            onClick={() => onSelect(cls)}
            style={{
              display: 'flex', alignItems: 'center', gap: 14,
              padding: '18px 20px', borderRadius: 18, textAlign: 'left',
              background: isActive ? `${style.color}10` : 'var(--bg-primary)',
              border: `2px solid ${isActive ? style.color : 'var(--border)'}`,
              cursor: 'pointer',
              transform: isActive ? 'scale(1.02)' : 'scale(1)',
              boxShadow: isActive ? `0 8px 28px ${style.shadow}` : 'none',
              transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)',
            }}
            onMouseEnter={e => { if (!isActive) { e.currentTarget.style.borderColor = style.color; gsap.to(e.currentTarget, { x: 4, duration: 0.2 }); } }}
            onMouseLeave={e => { if (!isActive) { e.currentTarget.style.borderColor = 'var(--border)'; gsap.to(e.currentTarget, { x: 0, duration: 0.2 }); } }}
          >
            {/* Class badge */}
            <div style={{
            ...
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 15, color: 'var(--text-primary)', marginBottom: 3 }}>
...
export default function Onboarding() {
  ...
  const isHoursStep = step === QUIZ_STEPS.length;         // 2
  const isClassStep = step === QUIZ_STEPS.length + 1;     // 3
  const currentQuiz = isQuizStep ? QUIZ_STEPS[step] : null;
  ...
  useEffect(() => {
    if (progressRef.current) {
      gsap.to(progressRef.current, { width: `${(step / TOTAL_STEPS) * 100}%`, duration: 0.5, ease: 'power2.inOut' });
    }
  }, [step]);
  ...
  const handleNext = async () => {
    if (isClassStep) {
      // Final step — save prefs + enroll + navigate
  ...
  const canAdvance = () => {
    if (isQuizStep)  return !!answers[currentQuiz.id];
    if (isHoursStep) return true;
    if (isClassStep) return !!selectedClass && !saving;
    return false;
  };
  ...
  if (done) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 24, padding: 24, position: 'relative', overflow: 'hidden' }}>
        ...
        <MovingWidgets />
        <div style={{ zIndex: 1, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20 }}>
          <div style={{ fontSize: 88, lineHeight: 1, animation: 'float2 3s ease-in-out infinite' }}>🎉</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 18px', borderRadius: 50, background: 'rgba(0,201,177,0.1)', border: '1px solid rgba(0,201,177,0.25)' }}>
              <CheckCircle size={15} color="#00c9b1" />
              <span style={{ fontSize: 12, fontWeight: 700, color: '#00c9b1' }}>Preferences saved & class joined!</span>
            </div>
            {selectedClass && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 18px', borderRadius: 50, background: `${(CLASS_COLORS[selectedClass.name] || defaultClassStyle).color}12`, border: `1px solid ${(CLASS_COLORS[selectedClass.name] || defaultClassStyle).color}25` }}>
                <School size={14} color={(CLASS_COLORS[selectedClass.name] || defaultClassStyle).color} />
                <span style={{ fontSize: 12, fontWeight: 700, color: (CLASS_COLORS[selectedClass.name] || defaultClassStyle).color }}>
          ...
          <p style={{ color: 'var(--text-secondary)', textAlign: 'center', maxWidth: 380, lineHeight: 1.75, fontSize: 15 }}>
            Your personalised CRPS study plan has been prepared based on your class subjects, study level, and board exam dates.
          </p>
          <button className="btn-primary"
            onClick={() => navigate('/dashboard')}
            onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.06, boxShadow: '0 12px 36px rgba(26,63,163,0.35)' })}
  ...
  // ── Step labels ───────────────────────────────────────────────────────
  const stepLabel = isQuizStep
    ? currentQuiz.title
    : isHoursStep
    ? 'Daily study hours?'
    : 'Select your class section';

  const stepEmoji = isQuizStep
    ? currentQuiz.emoji
    : isHoursStep
    ? '⏰'
    : '🏫';
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '40px 24px', position: 'relative', zIndex: 1 }}>
        <div style={{ maxWidth: 580, width: '100%', display: 'flex', flexDirection: 'column', gap: 28 }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10, justifyContent: 'center' }}>
              {Array.from({ length: TOTAL_STEPS }).map((_, i) => (
                <div key={i} style={{ width: i === step ? 24 : 8, height: 8, borderRadius: 4, background: i <= step ? '#1a3fa3' : 'var(--border)', transition: 'all 0.35s ease' }} />
              ))}
            </div>
          </div>
          ...
          <div ref={containerRef} className="card" style={{ padding: 44, boxShadow: '0 24px 70px rgba(0,0,0,0.07)', backdropFilter: 'blur(20px)', background: 'rgba(255,255,255,0.88)' }}>
            ...
            {/* Emoji + title */}
            <div style={{ fontSize: 56, marginBottom: 14, textAlign: 'center', lineHeight: 1 }}>{stepEmoji}</div>
            <h2 style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 28, textAlign: 'center', marginBottom: isClassStep ? 24 : 32, color: 'var(--text-primary)', lineHeight: 1.2 }}>
              {stepLabel}
            </h2>
            ...
            {isQuizStep && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {currentQuiz.options.map(opt => {
                  const selected = answers[currentQuiz.id] === opt.value;
                  return (
                    <button key={opt.value} data-val={opt.value} className="option-btn"
                      onClick={() => select(currentQuiz.id, opt.value)}
                      onMouseEnter={e => { if (!selected) gsap.to(e.currentTarget, { scale: 1.025, x: 4 }); }}
                      onMouseLeave={e => { if (!selected) gsap.to(e.currentTarget, { scale: 1, x: 0 }); }}
                      style={{
                        padding: '20px 26px', borderRadius: 18, textAlign: 'left',
                        background: selected ? 'rgba(26,63,163,0.07)' : 'var(--bg-primary)',
                        border: `2px solid ${selected ? '#1a3fa3' : 'var(--border)'}`,
                        cursor: 'pointer', transition: 'background 0.25s, border-color 0.25s',
                        transform: selected ? 'scale(1.02)' : 'scale(1)',
                        boxShadow: selected ? '0 10px 30px rgba(26,63,163,0.12)' : 'none',
                      }}>
                      <div style={{ fontWeight: 800, fontSize: 16, color: 'var(--text-primary)', marginBottom: 5 }}>{opt.label}</div>
                      <div style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.5 }}>{opt.desc}</div>
                    </button>
            ...
            {isHoursStep && (
              <>
                ...
                <input type="range" min={1} max={12} value={hours}
                  onChange={e => {
                    setHours(+e.target.value);
                    gsap.fromTo('.hour-pulse', { scale: 1.2 }, { scale: 1, duration: 0.3, ease: 'back.out(2)' });
                  }}
                  style={{ width: '100%', accentColor: 'var(--school-blue)', height: 8, cursor: 'pointer', borderRadius: 4 }}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 14, fontSize: 11, color: 'var(--text-muted)', fontWeight: 700 }}>
                ...
                {/* Preview */}
                <div style={{ marginTop: 28, padding: '16px 20px', borderRadius: 16, background: 'rgba(26,63,163,0.05)', border: '1px solid rgba(26,63,163,0.12)', display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                  {[
                    { label: 'Study Style', value: answers.level || '—', emoji: '🎯' },
                    { label: 'Goal',        value: answers.target || '—', emoji: '🏆' },
                    { label: 'Daily Hours', value: `${hours}h`,           emoji: '⏰' },
                  ].map((item, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span style={{ fontSize: 16 }}>{item.emoji}</span>
                      <div>
                        <p style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{item.label}</p>
                        <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', textTransform: 'capitalize' }}>{item.value}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {isClassStep && (
              <>
                <p style={{ textAlign: 'center', color: 'var(--text-muted)', marginBottom: 24, fontSize: 14 }}>
                  This links you to your teacher's subjects and study tasks
                </p>
                ...
                {saveError && (
                  <div style={{ marginTop: 16, padding: '12px 16px', borderRadius: 12, background: 'rgba(255,45,120,0.08)', border: '1px solid rgba(255,45,120,0.2)', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <AlertCircle size={15} color="#ff2d78" />
                    <span style={{ fontSize: 13, color: '#ff2d78' }}>{saveError}</span>
                  </div>
                )}

                {selectedClass && (
                  <div style={{ marginTop: 20, padding: '14px 18px', borderRadius: 16, background: `${(CLASS_COLORS[selectedClass.name] || defaultClassStyle).color}08`, border: `1px solid ${(CLASS_COLORS[selectedClass.name] || defaultClassStyle).color}20`, display: 'flex', alignItems: 'center', gap: 12, animation: 'slideUp 0.35s ease' }}>
                    <CheckCircle size={18} color={(CLASS_COLORS[selectedClass.name] || defaultClassStyle).color} />
                    <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>
                      Selected: <span style={{ color: (CLASS_COLORS[selectedClass.name] || defaultClassStyle).color }}>Class {selectedClass.name} — Section {selectedClass.section}</span>
                    </p>
                  </div>
                )}
              </>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
            <button
              ...
              onMouseEnter={e => step > 0 && gsap.to(e.currentTarget, { x: -5 })}
              onMouseLeave={e => gsap.to(e.currentTarget, { x: 0 })}
              style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '13px 26px', borderRadius: 14, background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'var(--text-secondary)', cursor: step === 0 ? 'not-allowed' : 'pointer', fontSize: 14, fontWeight: 600, opacity: step === 0 ? 0.4 : 1 }}
            >
              <ChevronLeft size={15} /> Back
            </button>
            ...
            <button
              ...
              onClick={handleNext}
              disabled={!canAdvance()}
              onMouseEnter={e => { if (canAdvance()) gsap.to(e.currentTarget, { scale: 1.05, y: -2 }); }}
              onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1, y: 0 })}
              style={{ opacity: !canAdvance() ? 0.4 : 1, fontSize: 14, padding: '13px 32px', fontWeight: 700, borderRadius: 14, display: 'flex', alignItems: 'center', gap: 8 }}
            >
              {saving ? (
                <><Loader size={15} style={{ animation: 'rotateSpin 0.8s linear infinite' }} /> Enrolling…</>
              ) : isClassStep ? (
          ...
          {/* Skip class step note */}
          {isClassStep && (
            <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-muted)' }}>
              Can't find your class?{' '}
              <button
                onClick={() => navigate('/dashboard')}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#1a3fa3', fontWeight: 600, fontSize: 12 }}
              >
                Skip for now
              </button>
              {' '}(you can enroll later from the dashboard)
            </p>
          )}
