...
// ✅ FIX Bug 35: Two loading-state problems fixed:
//   1. UI flashed an empty state briefly BEFORE shimmer appeared because `loading` started
//      as `true` but the grid rendered immediately with `filteredClasses` (which was []).
//      Fix: render the shimmer grid whenever `loading === true`, regardless of filteredClasses.
//   2. Shimmer count was hardcoded to 3.  Fix: show a shimmer for every section that currently
//      exists for the active tab (or a minimum of 2 if none exist yet), so the placeholder
//      layout matches the real content size.
...
import {
  GraduationCap, Plus, BookOpen, Trash2, Zap, Users,
  School, X, Check, Layers, RefreshCw, Award,
...
function FloatingBg({ activeColor }) {
  useEffect(() => {
    gsap.to('.td-float-blob', {
      y: () => `random(-60, 60)`, x: () => `random(-40, 40)`,
      scale: () => `random(0.9, 1.1)`, duration: () => `random(5, 9)`,
      repeat: -1, yoyo: true, ease: 'sine.inOut', stagger: 0.2,
    });
  }, []);
...
function CreateClassModal({ activeTab, onClose, onCreated }) {
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(10px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div ref={modalRef} style={{ background: 'var(--bg-card)', borderRadius: 28, padding: '40px', width: '90%', maxWidth: 440, border: `1px solid ${tab?.color}30`, boxShadow: `0 32px 100px rgba(0,0,0,0.35), 0 0 0 1px ${tab?.color}15`, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 4, background: tab?.gradient }} />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <h2 style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 20, color: 'var(--text-primary)' }}>Create {tab?.label}</h2>
            </div>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', paddingLeft: 48 }}>Pick a section for this class</p>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 4 }}><X size={20} /></button>
        </div>

        <div style={{ marginBottom: 24 }}>
          <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', letterSpacing: '0.1em', textTransform: 'uppercase', display: 'block', marginBottom: 12 }}>Select Section</label>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {['A', 'B', 'C', 'D'].map(s => (
              <button key={s} className="modal-btn-section" onClick={() => setSection(s)} style={{
                width: 54, height: 54, borderRadius: 14,
            ...
            <input placeholder="Other" value={!['A','B','C','D'].includes(section) ? section : ''} onChange={e => setSection(e.target.value.toUpperCase().slice(0,2))} maxLength={2}
              style={{ flex: 1, minWidth: 80, padding: '12px 16px', borderRadius: 14, border: `2px solid var(--border)`, background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: 15, fontWeight: 700, outline: 'none', fontFamily: 'Syne,sans-serif', textAlign: 'center' }}
              onFocus={e => e.target.style.borderColor = tab?.color}
              onBlur={e => e.target.style.borderColor = 'var(--border)'}
            />
          </div>
        ...
        <div style={{ padding: '14px 18px', borderRadius: 14, marginBottom: 28, background: `${tab?.color}08`, border: `1px solid ${tab?.color}20`, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: tab?.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><School size={16} color="white" /></div>
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{tab?.label} — Section {section}</p>
            <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>Add subjects & chapters after creating</p>
          </div>
        </div>

        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '13px', borderRadius: 14, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-secondary)', fontWeight: 600, cursor: 'pointer', fontSize: 14 }}>Cancel</button>
          <button onClick={handleCreate} disabled={loading || success} style={{
            flex: 2, padding: '13px', borderRadius: 14, border: 'none',
            background: success ? 'linear-gradient(135deg,#00c9b1,#00e6cc)' : tab?.gradient,
            color: 'white', fontWeight: 700, fontSize: 14, cursor: 'pointer',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
            boxShadow: `0 6px 24px ${tab?.shadow}`,
            transition: 'all 0.3s ease', opacity: loading ? 0.75 : 1,
          }}>
            {success ? <><Check size={16} /> Created!</>
              : loading ? <><RefreshCw size={14} style={{ animation: 'rotateSpin 1s linear infinite' }} /> Creating…</>
              : <><Plus size={14} /> Create Class</>}
          </button>
        </div>
      </div>
...
function ClassCard({ cls, tab, onAddSubject, onDelete }) {
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Delete Class ${cls.name}-${cls.section} and all its subjects?`)) return;
    setDeleting(true);
    gsap.to(cardRef.current, { scale: 0.85, opacity: 0, y: -20, duration: 0.35, ease: 'power2.in', onComplete: () => onDelete(cls._id) });
  };
      ...
      <div style={{ padding: '18px 22px' }}>
        {cls.subjects?.length > 0 ? (
          <div style={{ marginBottom: 16 }}>
            ...
            {cls.subjects.length > 4 && <p style={{ fontSize: 11, color: 'var(--text-muted)', paddingTop: 8, fontWeight: 500 }}>+{cls.subjects.length - 4} more subjects</p>}
          </div>
        ) : (
          <div style={{ padding: '20px 0', textAlign: 'center', marginBottom: 12 }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: `${tab.color}10`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 10px' }}><BookOpen size={20} color={tab.color} /></div>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 500 }}>No subjects yet</p>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>Add the first subject below</p>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => onAddSubject(cls._id)} style={{ flex: 1, padding: '10px 12px', borderRadius: 12, background: `${tab.color}10`, border: `1px solid ${tab.color}25`, color: tab.color, fontWeight: 700, fontSize: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, transition: 'all 0.25s ease' }}
            onMouseEnter={e => { e.currentTarget.style.background = `${tab.color}20`; gsap.to(e.currentTarget, { scale: 1.03 }); }}
            onMouseLeave={e => { e.currentTarget.style.background = `${tab.color}10`; gsap.to(e.currentTarget, { scale: 1 }); }}
          ><Plus size={13} /> Add Subject</button>

          <button onClick={handleGenerate} disabled={generating || !cls.subjects?.length} style={{
            flex: 1, padding: '10px 12px', borderRadius: 12,
            background: generated ? 'rgba(0,201,177,0.1)' : 'rgba(232,160,32,0.08)',
            border: `1px solid ${generated ? 'rgba(0,201,177,0.3)' : 'rgba(232,160,32,0.2)'}`,
            color: generated ? '#00c9b1' : '#c9820a', fontWeight: 700, fontSize: 12,
            cursor: cls.subjects?.length ? 'pointer' : 'not-allowed',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
            opacity: !cls.subjects?.length ? 0.45 : 1, transition: 'all 0.3s ease',
          }}>
            {generated ? <><Check size={12} /> Done!</>
...
const TeacherDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('9');
  const [showModal, setShowModal] = useState(false);
  const [loading, setLoading]     = useState(true);   // ✅ Bug 35: true on first render
  const [error, setError]         = useState('');
  const { userName }              = useApp();
  const navigate                  = useNavigate();
  const headerRef                 = useRef();
  ...
  // Animate cards when tab or data changes (only when NOT loading)
  useEffect(() => {
    if (loading) return;
    gsap.fromTo('.class-grid-card',
      { opacity: 0, y: 28, scale: 0.94 },
  ...
  const fetchClasses = async () => {
    setLoading(true);   // ✅ Bug 35: set loading BEFORE clearing error, so shimmer shows immediately
    setError('');
    try {
      const data = await teacherService.getClasses();
      setClasses(data);
  ...
  // ✅ Bug 35: dynamic shimmer count based on known data (or minimum 2)
  const shimmerCount = filteredClasses.length > 0 ? filteredClasses.length : 2;
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '36px 28px', position: 'relative', zIndex: 1 }}>
        ...
        <div ref={headerRef} style={{ marginBottom: 36 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 20, marginBottom: 28 }}>
            ...
            <button onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '14px 26px', borderRadius: 16, border: 'none', background: tab?.gradient, color: 'white', fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: `0 8px 28px ${tab?.shadow}`, transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}
              onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.06, y: -2 })}
              onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1, y: 0 })}
            ><Plus size={17} /> Add {tab?.label} Section</button>
          </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 28, flexWrap: 'wrap' }}>
          {CLASS_TABS.map(t => (
            <button key={t.num} className="class-tab-btn" onClick={() => setActiveTab(t.num)} style={{
              display: 'flex', alignItems: 'center', gap: 9, padding: '12px 26px', borderRadius: 50,
              background: activeTab === t.num ? t.gradient : 'var(--bg-card)',
              color: activeTab === t.num ? 'white' : 'var(--text-secondary)',
              border: `2px solid ${activeTab === t.num ? t.color : 'var(--border)'}`,
              fontFamily: 'Syne,sans-serif', fontWeight: 700, fontSize: 14, cursor: 'pointer',
              ...
              <School size={15} />
              {t.label}
              <span style={{ fontSize: 11, fontWeight: 800, background: activeTab === t.num ? 'rgba(255,255,255,0.22)' : `${t.color}14`, color: activeTab === t.num ? 'white' : t.color, padding: '2px 9px', borderRadius: 20, minWidth: 22, textAlign: 'center' }}>
                {classes.filter(c => c.name === t.num).length}
              </span>
            </button>
          ))}
        ...
        {/* ── Context banner ── */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', borderRadius: 16, marginBottom: 28, background: `${tab?.color}06`, border: `1px solid ${tab?.color}18`, flexWrap: 'wrap', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Award size={16} color={tab?.color} />
            <span style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>
              <strong style={{ color: tab?.color }}>{tab?.label}</strong> ·{' '}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={fetchClasses} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', background: 'var(--bg-card)', border: '1px solid var(--border)', padding: '6px 14px', borderRadius: 20, cursor: 'pointer' }}>
              <RefreshCw size={12} /> Refresh
            </button>
            {!loading && filteredClasses.length === 0 && (
              <button onClick={() => setShowModal(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 700, color: tab?.color, background: `${tab?.color}10`, border: `1px solid ${tab?.color}25`, padding: '6px 14px', borderRadius: 20, cursor: 'pointer' }}>
                <Plus size={12} /> Create First Section
              </button>
            )}
          </div>
        ...
        {/* ── Error banner ── */}
        {error && (
          <div style={{ padding: '14px 20px', borderRadius: 14, marginBottom: 24, background: 'rgba(255,45,120,0.08)', border: '1px solid rgba(255,45,120,0.2)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <AlertCircle size={16} color="#ff2d78" />
            <span style={{ fontSize: 13, color: '#ff2d78' }}>{error}</span>
          </div>
        )}
        ...
        {/* ── Class grid ── */}
        {loading ? (
          // ✅ Bug 35: shimmer renders immediately when loading=true — no empty flash.
          //           Count matches expected cards (known existing + 1 for the "add" card).
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px,1fr))', gap: 20 }}>
            {Array.from({ length: shimmerCount }).map((_, i) => (
              <div key={i} className="shimmer-bar" style={{ height: 280, borderRadius: 22 }} />
            ))}
          </div>
        ) : filteredClasses.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '90px 40px', background: 'var(--bg-card)', borderRadius: 28, border: `2px dashed ${tab?.color}30`, animation: 'slideUp 0.5s ease both' }}>
            <div style={{ width: 90, height: 90, borderRadius: 26, margin: '0 auto 22px', background: `${tab?.color}10`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <School size={40} color={tab?.color} />
            </div>
            <h3 style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 22, color: 'var(--text-primary)', marginBottom: 10 }}>No {tab?.label} sections yet</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 28, maxWidth: 340, margin: '0 auto 28px' }}>
              Create your first {tab?.label} section to start adding subjects, chapters, and generating AI study tasks.
            </p>
            <button onClick={() => setShowModal(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, padding: '14px 30px', borderRadius: 16, border: 'none', background: tab?.gradient, color: 'white', fontWeight: 700, fontSize: 15, cursor: 'pointer', boxShadow: `0 8px 28px ${tab?.shadow}` }}
              onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.05 })}
              onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1 })}
            ><Sparkles size={17} /> Create {tab?.label} Section</button>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px,1fr))', gap: 20 }}>
            {filteredClasses.map(cls => (
            ...
            {/* Add section tile */}
            <button onClick={() => setShowModal(true)} style={{ borderRadius: 22, border: `2px dashed ${tab?.color}35`, background: 'transparent', cursor: 'pointer', padding: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, minHeight: 200, transition: 'all 0.3s ease' }}
              onMouseEnter={e => { e.currentTarget.style.background = `${tab?.color}06`; e.currentTarget.style.borderColor = tab?.color; gsap.to(e.currentTarget, { scale: 1.02 }); }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.borderColor = `${tab?.color}35`; gsap.to(e.currentTarget, { scale: 1 }); }}
            >
              <div style={{ width: 56, height: 56, borderRadius: 16, background: `${tab?.color}12`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Plus size={26} color={tab?.color} /></div>
              <div style={{ textAlign: 'center' }}>
                <p style={{ fontFamily: 'Syne,sans-serif', fontWeight: 700, fontSize: 15, color: 'var(--text-primary)', marginBottom: 4 }}>Add Another Section</p>
                <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Create {tab?.label} - Section B, C, D…</p>
              </div>
            </button>
          </div>
