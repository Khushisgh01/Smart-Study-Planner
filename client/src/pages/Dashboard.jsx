...
//         // Don't flash modal — check the user's actual classId instead
//         const token = localStorage.getItem('token');
//         const userRes = await fetch('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } });
//         // If /api/auth/me doesn't exist yet, just don't show modal on empty subjects
//         // A teacher will add subjects later
//         setIsEnrolled(true);  // optimistic — let the empty state handle it
//       } catch {
//         // ignore
//       }
//     };
...
import {
  Zap, Calendar, TrendingUp, ShieldAlert, ChevronRight,
  BookOpen, School, CheckCircle, X, Loader,
} from 'lucide-react';
        ...
        <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
          ...
          <circle
            ...
            style={{
              ...
              filter: `drop-shadow(0 0 7px ${color}70)`,
            }}
          />
        </svg>
        <div style={{
          ...
          <span style={{
            ...
            fontSize: size * 0.21, color: 'var(--text-primary)',
          }}>
            {value}%
          </span>
        </div>
...
// ── Enrollment modal ──────────────────────────────────────────────────────
function EnrollmentModal({ onEnrolled, onClose, currentClassId = null }) {
  const [classes,   setClasses]   = useState([]);
  const [loading,   setLoading]   = useState(true);
  const [enrolling, setEnrolling] = useState(null);
  const [done,      setDone]      = useState(false);
  const [error,     setError]     = useState('');
  const modalRef = useRef();
  const isSwitch = Boolean(currentClassId);
  ...
  // Animate class items once they load
  useEffect(() => {
  ...
  return (
    <div style={{
      ...
      <div ref={modalRef} style={{
        background: 'var(--bg-card)', borderRadius: 28, padding: '36px',
        width: '100%', maxWidth: 480,
        border: '1px solid var(--border)', boxShadow: '0 32px 100px rgba(0,0,0,0.3)',
        position: 'relative',
      }}>
        {onClose && (
          <button
            ...
            style={{
              ...
              borderRadius: 10, padding: 6, cursor: 'pointer', color: 'var(--text-muted)',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
            }}
          >
            <X size={16} />
        ...
        {/* Gradient top bar */}
        <div style={{
          height: 4, background: 'linear-gradient(90deg,#1a3fa3,#00c9b1)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <div>
            <h2 style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 20, color: 'var(--text-primary)' }}>
              {done ? (isSwitch ? 'Class Updated! 🎉' : 'Enrolled! 🎉') : (isSwitch ? 'Switch Class' : 'Join Your Class')}
            </h2>
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              {done
                ? 'Refreshing your dashboard…'
                : isSwitch
                  ? 'Pick a different section — your old tasks will be cleared'
                  : 'Select your class section to get started'}
            </p>
          </div>
        </div>
        ...
        {error && (
          <div style={{
            ...
            fontSize: 13, color: '#ff2d78', display: 'flex', alignItems: 'center', gap: 8,
          }}>
            <X size={14} /> {error}
          </div>
        )}
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
              You're enrolled! Your study plan is being prepared.
            </p>
          </div>
          ...
          <div style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            padding: '40px 0', gap: 10, color: 'var(--text-muted)',
          }}>
            <Loader size={18} style={{ animation: 'rotateSpin 0.8s linear infinite' }} />
            <span style={{ fontSize: 13 }}>Loading available classes…</span>
          </div>
          ...
          <div style={{
            textAlign: 'center', padding: '32px', borderRadius: 16,
            border: '2px dashed var(--border)',
          }}>
            <School size={32} color="var(--text-muted)"
          ...
          <div style={{
            ...
            {classes.map(cls => {
              const color       = CLASS_COLORS[cls.name] || '#1a3fa3';
              const isEnrolling = enrolling === cls._id;
              const isCurrent   = currentClassId && cls._id === currentClassId;
              return (
                <button
                  key={cls._id}
                  className="enroll-item"
                  onClick={() => !isCurrent && handleEnroll(cls._id)}
                  disabled={!!enrolling || isCurrent}
                  style={{
                    display: 'flex', alignItems: 'center', gap: 14,
                    padding: '16px 18px', borderRadius: 16, textAlign: 'left',
                    background: isCurrent ? `${color}18` : `${color}06`,
                    border: `1.5px solid ${isCurrent ? color : `${color}22`}`,
                    cursor: enrolling || isCurrent ? 'default' : 'pointer',
                    opacity: enrolling && !isEnrolling ? 0.5 : 1,
                    transition: 'all 0.25s ease',
                  }}
                  onMouseEnter={e => {
                    if (!enrolling && !isCurrent) {
                      e.currentTarget.style.background    = `${color}12`;
                      e.currentTarget.style.borderColor   = color;
                    }
                  }}
                  onMouseLeave={e => {
                    e.currentTarget.style.background  = isCurrent ? `${color}18` : `${color}06`;
                    e.currentTarget.style.borderColor = isCurrent ? color : `${color}22`;
                  }}
                >
                  <div style={{
                    width: 42, height: 42, borderRadius: 12, background: color,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    flexShrink: 0, boxShadow: `0 4px 14px ${color}40`,
                  }}>
                    <School size={18} color="white" />
                  </div>
                  <div style={{ flex: 1 }}>
                  ...
                  {isCurrent ? (
                    <span style={{ fontSize: 10, fontWeight: 700, color, background: `${color}15`, padding: '4px 10px', borderRadius: 20 }}>
                      Current
                    </span>
                  ) : isEnrolling ? (
                    <Loader size={16} color={color} style={{ animation: 'rotateSpin 0.8s linear infinite' }} />
...
export default function Dashboard() {
  ...
  // Refresh subjects/tasks whenever the dashboard opens (e.g. after onboarding)
  useEffect(() => {
    fetchSubjects?.();
    fetchTodayTasks?.();
    fetchTomorrowTasks?.();
  ...
  useEffect(() => {
    const loadEnrollment = async () => {
      try {
        ...
        const classId = me.classId?._id || me.classId || null;
        setEnrolledClassId(classId ? String(classId) : null);
        if (classId) {
          ...
          if (classRes.ok) {
            ...
            setEnrolledClassLabel(`Class ${cls.name} — Section ${cls.section}`);
          }
        } else {
          setEnrolledClassLabel('');
        }
      } catch {
        // non-blocking
      }
  ...
  const handleEnrolled = useCallback(() => {
    ...
    fetchTodayTasks?.();
    fetchTomorrowTasks?.();
    // Reload enrollment label after switch
    const token = localStorage.getItem('token');
    if (token) {
      fetch('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } })
        ...
        .then(me => {
          ...
          const id = me.classId._id || me.classId;
          setEnrolledClassId(String(id));
          return fetch(`/api/class/${id}`, { headers: { Authorization: `Bearer ${token}` } });
        })
        .then(r => r?.ok ? r.json() : null)
        .then(cls => cls && setEnrolledClassLabel(`Class ${cls.name} — Section ${cls.section}`));
    }
  }, [fetchSubjects, fetchTodayTasks, fetchTomorrowTasks]);
    ...
    <div style={{ display: 'flex', flex: 1, minHeight: 0, overflow: 'hidden' }}>
      ...
      {showEnrollModal && (
        <EnrollmentModal
          ...
          currentClassId={enrolledClassId}
        />
      )}

      <Sidebar onSwitchClass={() => setShowEnrollModal(true)} />
        ...
        >
          <div>
            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 3 }}>
              {today} · Chhotu Ram Public School
            </p>
            <h1 style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 26, color: 'var(--text-primary)' }}>
              Good morning, <span style={{ color: 'var(--school-blue)' }}>{getDisplayName(userName, 'Student')}</span>! 👋
            </h1>
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 5 }}>
              {enrolledClassLabel && (
                <span style={{
                  display: 'inline-flex', alignItems: 'center', gap: 6,
                  marginRight: 10, padding: '3px 10px', borderRadius: 20,
                  background: 'rgba(26,63,163,0.08)', color: 'var(--school-blue)',
                  fontSize: 11, fontWeight: 700,
                }}>
                  <School size={12} /> {enrolledClassLabel}
                </span>
              )}
              {subjects.length === 0
                ? 'Enroll in your class to see your personalised study plan'
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
            <button
              ...
              style={{
                ...
                border: subjects.length > 0 ? '1px solid var(--border)' : 'none',
                background: subjects.length > 0
                  ? 'var(--bg-card)'
                  : 'linear-gradient(135deg,#1a3fa3,#3a5fd4)',
                color: subjects.length > 0 ? 'var(--text-primary)' : 'white',
                fontWeight: 700, fontSize: 13, cursor: 'pointer',
                boxShadow: subjects.length > 0 ? 'none' : '0 6px 20px rgba(26,63,163,0.3)',
              }}
              onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.04, y: -2 })}
              onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1, y: 0 })}
            >
              <School size={15} />
              {subjects.length > 0 || enrolledClassId ? 'Switch Class' : 'Join a Class'}
            </button>

            {/* Stat chips — only when enrolled with subjects */}
            {subjects.length > 0 && [
              { label: 'Done',      value: doneCount,    color: '#00c9b1', bg: 'rgba(0,201,177,0.08)'  },
              { label: 'Remaining', value: pendingCount, color: '#1a3fa3', bg: 'rgba(26,63,163,0.08)'  },
              { label: 'Minutes',   value: totalMins,    color: '#e8a020', bg: 'rgba(232,160,32,0.10)' },
              ...
              >
                <p style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 24, color: s.color }}>
                  {s.value}
                </p>
                <p style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>{s.label}</p>
              </div>
        ...
        {subjects.length === 0 ? (
          <div
            ...
            style={{
              display: 'flex', flexDirection: 'column',
              alignItems: 'center', justifyContent: 'center',
              minHeight: 400, gap: 24, textAlign: 'center',
          ...
          >
            ...
            <div>
              <h2 style={{
                fontFamily: 'Syne,sans-serif', fontWeight: 800,
                fontSize: 24, color: 'var(--text-primary)', marginBottom: 10,
              }}>
                {enrolledClassId ? 'No subjects in your class yet' : "You're not enrolled in a class yet"}
              </h2>
              <p style={{ color: 'var(--text-muted)', maxWidth: 400, margin: '0 auto', lineHeight: 1.7, fontSize: 14 }}>
                {enrolledClassId
                  ? 'Your teacher may still be adding subjects, or you can switch to another class section.'
                  : 'Once you join your class section, your teacher\'s subjects and personalised study tasks will appear here automatically.'}
              </p>
            </div>
            <button
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 330px', gap: 20 }}>
              ...
              <div className="card dash-section" style={{ padding: '22px' }}>
                ...
                <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                  {[
                    ...
                    <div
                      key={i}
                      className="prog-ring-wrap"
                      onMouseEnter={e => gsap.to(e.currentTarget, { scale: 1.08, duration: 0.3, ease: 'back.out(1.5)' })}
                      onMouseLeave={e => gsap.to(e.currentTarget, { scale: 1,    duration: 0.3 })}
                    >
                      <ProgressRing
                        value={r.value} size={96}
                        color={r.color} label={r.label} sublabel={r.sub}
                      />
                    </div>
                  ))}
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                {tomorrowTasks.length === 0 ? (
                  ...
                  >
                    <div style={{
                      ...
                      background: (
                        { Learn: '#1a3fa3', Revise1: '#9b59b6', Revise2: '#7d3b9c', PYQ: '#e8a020' }
                      )[t.taskType] ?? '#aaa',
                    }} />
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 10, color: 'var(--text-muted)' }}>
                        {t.subject} · {t.estimatedTime} min
                      </p>
                    </div>
                    <span style={{
                      fontSize: 9, fontWeight: 700,
                      color: '#1a3fa3', background: 'rgba(26,63,163,0.1)',
              ...
              <div
                ...
                style={{
                  padding: '18px',
                  background: 'linear-gradient(135deg, rgba(26,63,163,0.04), rgba(232,160,32,0.04))',
                }}
              >
                <SectionHeader icon={BookOpen} title="PYQ Repository" color="#1a3fa3" />
                <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 14, lineHeight: 1.65 }}>
