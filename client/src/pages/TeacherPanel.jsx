...
import { Plus, Trash2, Save, GraduationCap, BookOpen, FileText } from 'lucide-react';
...
export default function TeacherPanel() {
  ...
  const removeSubject = (si) => {
    gsap.to(`.subject-block:nth-child(${si + 1})`, { opacity: 0, scale: 0.9, x: -20, duration: 0.3, ease: 'power2.in', onComplete: () => setSubjects(s => s.filter((_,i) => i !== si)) });
  };
    ...
    <div style={{ flex: 1, overflowY: 'auto', padding: '28px 32px' }}>
      ...
      <div ref={headerRef} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          ...
          <button onClick={handleSave} className="btn-primary" style={{ padding: '9px 20px', fontSize: 13 }}>
            <Save size={14} /> Save & Generate
          </button>
        </div>
      </div>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  {SUBJECT_COLORS.map(c => (
                    <button key={c} onClick={() => updateSubject(si, 'color', c)} style={{
                      width: 18, height: 18, borderRadius: '50%', background: c, border: sub.color === c ? `2px solid var(--text-primary)` : '2px solid transparent',
