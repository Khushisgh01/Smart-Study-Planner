...
//     if (daysLeft <= 21 && s.myProgress?.pct ?? 0 < 50) {
...
//     if (daysLeft <= 5 && s.myProgress?.pct ?? 0 < 80) {
//       risks.push({
//         chapter: `${s.name} overall`,
//         marks: Math.round((100 - (s.myProgress?.pct ?? 0)) / 10) * 3,
//         subject: s.name,
//         daysLeft,
//         severity: 'critical',
//       });
...
function buildRisks(subjects, tasks) {
  ...
  subjects.forEach(s => {
    ...
    const pct = s.myProgress?.pct ?? 0;

    if (daysLeft <= 21 && pct < 50) {
      const worstChapter = (s.chapters || [])
        .sort((a, b) => (b.weightage || 0) - (a.weightage || 0))[0];
    ...
    cards.forEach((card, i) => {
      if (risks[i]?.severity === 'critical') {
        gsap.to(card, {
          ...
          duration: 1.5, repeat: -1, yoyo: true, ease: 'sine.inOut', delay: i * 0.3,
        });
      }
    });
  }, [risks.length, subjects.length, tasks.length]);
      ...
      {risks.map((r, i) => {
        ...
        return (
          <div
            key={i}
            style={{
              padding: '12px 16px', borderRadius: 12, background: bgColor,
            ...
            onMouseEnter={e => {
              e.currentTarget.style.background = isCritical ? 'rgba(255,45,120,0.13)' : 'rgba(232,160,32,0.13)';
          ...
          >
            ...
            {isCritical && (
              <span style={{
                ...
                padding: '2px 8px', borderRadius: 20, flexShrink: 0, alignSelf: 'center',
              }}>
                URGENT
              </span>
            )}
