...
function buildInsights(subjects, tasks) {
  ...
  const lagging = subjects
    .map(s => ({
      ...s,
      pct: s.myProgress?.pct ?? 0,
      daysLeft: Math.ceil((new Date(s.examDate) - new Date()) / 86400000),
    }))
    .filter(s => s.daysLeft > 0 && s.pct < 50)
    .sort((a, b) => a.daysLeft - b.daysLeft)[0];

  if (lagging) {
    insights.push({
      text: `${lagging.name} is only ${lagging.pct}% done with ${lagging.daysLeft} days to exam — prioritise it now.`,
      icon: AlertCircle, color: '#ff2d78',
    });
  }

  // 3 — High-progress encouragement
  const winning = subjects
    .map(s => ({ ...s, pct: s.myProgress?.pct ?? 0 }))
    .filter(s => s.pct >= 80)[0];

  if (winning && !lagging) {
    insights.push({
      text: `${winning.name} is at ${winning.pct}% — excellent progress! Consider a PYQ session to lock it in.`,
      icon: Star, color: '#00c9b1',
    });
  }
    ...
    <div ref={containerRef} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {insights.map((ins, i) => (
        <div
          ...
          onMouseEnter={e => {
            e.currentTarget.style.background = `${ins.color}12`;
            gsap.to(e.currentTarget, { x: 3, duration: 0.25 });
          }}
          onMouseLeave={e => {
            e.currentTarget.style.background = `${ins.color}08`;
