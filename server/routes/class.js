...
// ── GET /api/class/:id ────────────────────────────────────────────────────
// FIX: this endpoint was entirely missing — any flow that fetches a single
//      class by ID (e.g. student class detail, teacher edit view) would 404.
//
// Accessible by the owning teacher OR any enrolled student (classId match).
// Returns full class with subjects populated.
router.get('/:id', protect, async (req, res) => {
  try {
    const cls = await Class.findById(req.params.id)
      .populate({ path: 'subjects', select: 'name color examDate chapters pyqs' })
      .lean();

    if (!cls) return res.status(404).json({ message: 'Class not found' });

    // Gate: teacher must own it, or student must be enrolled in it
    const isTeacher = req.user.role === 'teacher' && cls.teacher.toString() === req.user.id;
    const isStudent = req.user.role !== 'teacher'; // role comes from JWT, not DB
...
// Teacher: list all students enrolled in a class, with their task progress.
//
// FIX: was querying User.find({ classId, role: 'student' }) — User model has
//      no role field (role lives only in the JWT), so this always returned [].
//      Now queries by classId alone; Teacher accounts use a separate
//      collection (Teacher model) so any User with classId IS a student.
router.get('/:id/students', protect, teacherOnly, async (req, res) => {
  try {
    const cls = await Class.findOne({ _id: req.params.id, teacher: req.user.id });
    if (!cls) return res.status(404).json({ message: 'Class not found' });

    // FIX: removed `role: 'student'` — field does not exist on User model
...
// Teacher: detailed chapter-level progress for a specific student.
//
// FIX: same role:'student' issue removed from User.findOne here too.
router.get('/:id/students/:studentId/progress', protect, teacherOnly, async (req, res) => {
  try {
    const cls = await Class.findOne({ _id: req.params.id, teacher: req.user.id });
    if (!cls) return res.status(404).json({ message: 'Class not found' });

    // FIX: removed `role: 'student'` filter
    const student = await User.findOne({
      _id:     req.params.studentId,
      classId: req.params.id,
    }).select('name email');
...
router.delete('/:id', protect, teacherOnly, async (req, res) => {
  try {
    ...
    // Collect enrolled student IDs BEFORE unenrolling them
