...
// Mongoose 9: use async middleware — callback-style next() is no longer supported
subjectSchema.pre('save', async function () {
  ...
  const now = Date.now();
  const sevenDay = 7 * 24 * 60 * 60 * 1000;
  const ed = this.examDate ? new Date(this.examDate) : null;

  if (ed && !isNaN(ed.getTime()) && (ed.getTime() - now) <= sevenDay && ed.getTime() > now) {
    this.badge = 'Urgent';
  } else if (this.chapters?.some(c => (c.weightage || 0) >= 8)) {
    this.badge = 'High Weight';
  } else if (this.chapters?.some(c => c.isWeak)) {
